...
mod test_statistics {
    ...
    fn test_simple_moving_average() {
        ...
        //Test SMA with window size 4
        let result = simple_moving_average(&data_set, 4).unwrap();
        assert_eq!(1, result.len());
        assert_eq!(vec![4.25], result);

        //Test SMA with window size  bigger than data set, should return None
    ...
    fn test_exponential_moving_average() {
        ...
        assert_eq!(3, result.len());
        assert_eq!(vec![22.220999999999997, 22.208090909090906, 22.241165289256195], result);
    }

    #[test]
    fn test_moving_average_convergence_divergence() {
        let data_set = vec![5.0, 6.0, 4.0, 2.0, 1.5, 1.0, 2.0, 3.0, 3.5, 3.5, 4.0, 4.5, 5.0];
        ...
        //result is a struct containing two Vec<f64>, one for macd and one for signal.
    ...
    fn test_bollinger_bands() {
        ...
        assert_eq!(vec![3.0625, 2.875, 2.5625, 2.5625, 2.875, 3.3125], result.middle_bound);
    ...
    fn test_relative_strength_index() {
        ...
        assert_eq!(3, result.len());
        assert_eq!(vec![70.53539393736207, 66.436571546019, 66.66146763681454], result);
    }
}
